<x-app-layout>
                ...
                <div class="lg:col-span-1">
                    <div class="bg-white overflow-hidden shadow-sm rounded-xl border border-gray-100 sticky top-6">
                        <div class="p-6">
                            ...
                            <!-- Alert Messages -->
                            @if (session('success'))
                                <div class="mb-4 bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-lg text-sm relative">
                                    {{ session('success') }}
                                </div>
                            @endif
                            @if (session('error'))
                            ...
                            <form method="POST" action="{{ route('diagnosis.quick-diagnose') }}">
                                ...
                                <!-- Pilih Pasien -->
                                <div class="mb-6">
                                    <label class="block font-semibold text-sm text-gray-700 mb-2">Pilih Pasien</label>
                                    <div class="relative">
                                        <select name="patient_id_display" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 py-2.5 pl-3 pr-10 text-sm">
                                            <option value="">-- Mode Tamu (Tanpa Simpan) --</option>
                                            @foreach($patients as $p)
                                                <option value="{{ $p->id }}" {{ request('patient_id_display') == $p->id ? 'selected' : '' }}>
                                                    {{ $p->name }} ({{ $p->age }} Thn)
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <p class="text-xs text-gray-500 mt-2 flex items-start">
                                        <svg class="w-4 h-4 mr-1 text-yellow-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                        Wajib dipilih jika ingin menyimpan hasil ke Rekam Medis.
                                    </p>
                                </div>

                                <!-- Pilih Gejala -->
                                <div>
                                    <label class="block font-semibold text-sm text-gray-700 mb-2">Gejala yang Dialami</label>
                                    <div class="border border-gray-200 rounded-xl max-h-[400px] overflow-y-auto bg-gray-50 p-2 space-y-1 custom-scrollbar">
                                        @forelse($symptoms as $symptom)
                                            <label class="flex items-start p-2.5 rounded-lg cursor-pointer transition-colors hover:bg-white hover:shadow-sm">
                                ...
                                <button type="submit" class="mt-6 w-full flex justify-center py-3 px-4 border border-transparent rounded-xl shadow-sm text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all shadow-indigo-500/30">
                                    Analisa Gejala
                                </button>
                            </form>
                        </div>
                    ...
                    <div class="bg-white overflow-hidden shadow-sm rounded-xl border border-gray-100 min-h-[500px]">
                        <div class="p-8">
                            <h3 class="text-xl font-bold text-gray-900 mb-6 flex items-center">
                                <svg class="w-6 h-6 text-rose-500 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                ...
                                <div class="flex flex-col items-center justify-center py-12 text-center">
                                    ...
                                    <p class="text-gray-500 max-w-sm mt-2">
                                        @if(empty($selectedSymptoms))
                                            Silakan pilih pasien dan centang gejala yang dialami pada panel di sebelah kiri untuk memulai diagnosa.
                                        @else
                            ...
                            @else
                                <div class="space-y-6">
                                    @foreach($results as $disease)
                                        <div class="border border-gray-200 rounded-xl p-6 hover:shadow-md transition-shadow duration-300 relative overflow-hidden group">
                                            ...
                                            <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
                                                <div class="flex-1">
                                                    ...
                                                    <p class="text-gray-600 leading-relaxed mb-4">{{ $disease->description }}</p>

                                                    <div class="bg-gray-50 rounded-lg p-4 mt-4">
                                                        <span class="text-xs font-bold text-gray-500 uppercase tracking-wide">Gejala yang Cocok:</span>
                                                        <div class="flex flex-wrap gap-2 mt-2">
                                                            @foreach($disease->symptoms as $symptom)
                                                                <span class="{{ in_array($symptom->id, $selectedSymptoms) ? 'bg-indigo-100 text-indigo-700 border-indigo-200' : 'bg-gray-200 text-gray-500 border-gray-200' }} border px-3 py-1 rounded-full text-xs font-semibold">
                                                                    {{ $symptom->name }}
                                                ...
                                                <div class="md:w-48 flex-shrink-0 flex flex-col justify-end h-full">
                                                    <form action="{{ route('diagnosis.save') }}" method="POST" class="mt-4 md:mt-0">
                                                        ...
                                                        @php
                                                            $symptomNames = \App\Models\Symptom::whereIn('id', $selectedSymptoms)->pluck('name')->implode(', ');
                                                        @endphp
                                                        ...
                                                        <button type="button"
                                                            onclick="submitDiagnosis({{ $disease->id }})"
    ...
    <script>
        function submitDiagnosis(diseaseId) {
            ...
            var selectedPatient = document.querySelector('select[name="patient_id_display"]').value;

            // 2. Validasi
            if (!selectedPatient) {
                // Tampilkan alert yang lebih bagus (opsional, disini pakai alert biasa dulu)
                alert("Mohon Maaf:\n\nAnda harus memilih data 'Pasien' pada kolom kiri terlebih dahulu sebelum dapat menyimpan Rekam Medis.");

                // Highlight dropdown agar user sadar
                const selectElem = document.querySelector('select[name="patient_id_display"]');
                selectElem.focus();
                selectElem.classList.add('ring-2', 'ring-rose-500', 'border-rose-500');
                return;
            }
